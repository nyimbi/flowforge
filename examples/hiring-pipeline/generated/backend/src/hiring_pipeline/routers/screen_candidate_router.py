"""FastAPI router for Screen a candidate.

Exposes ``POST /screen-candidate/events`` so a frontend step
...
from ..screen_candidate.idempotency import (
	IDEMPOTENCY_TTL_HOURS,
	check_idempotency_key,
	fingerprint_request,
	record_idempotency_response,
)
from ..services.screen_candidate_service import ScreenCandidateService
...
router = APIRouter(prefix="/screen-candidate", tags=["screen_candidate"])
_service = ScreenCandidateService()
...
async def post_event(
	...
	span_attrs: dict[str, Any] = {
		...
		"flowforge.jtbd_id": "screen_candidate",
