"""FastAPI router for Source a candidate.

Exposes ``POST /source-candidate/events`` so a frontend step
...
from ..source_candidate.idempotency import (
	IDEMPOTENCY_TTL_HOURS,
	check_idempotency_key,
	fingerprint_request,
	record_idempotency_response,
)
from ..services.source_candidate_service import SourceCandidateService
...
router = APIRouter(prefix="/source-candidate", tags=["source_candidate"])
_service = SourceCandidateService()
...
async def post_event(
	...
	span_attrs: dict[str, Any] = {
		...
		"flowforge.jtbd_id": "source_candidate",
