"""Idempotency dedupe helper for Submit a Building Permit Application (v0.3.0 W2 / item 6).

Generated by flowforge JTBD generator — do not edit by hand.

Used by the FastAPI router and domain service to enforce
``Idempotency-Key`` on ``POST /permit-intake/events``:
...
The helper writes the per-JTBD ``permit_intake_idempotency_keys``
...
IDEMPOTENCY_TABLE: str = "permit_intake_idempotency_keys"
...
async def check_idempotency_key(
	*,
	...
	raise NotImplementedError(
		...
		"See backend/src/building_permit/permit_intake/idempotency.py "
