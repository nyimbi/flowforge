...
`S3DocumentPortInMemory` implements `flowforge.ports.DocumentPort` over an S3 bucket. The blob layer is durable (S3 objects); by default the per-doc metadata and subject index live in process memory. Hosts that need durable indexing can pass `SQLiteDocumentIndex`, a stdlib-SQLite metadata/index store that survives process restarts without adding package dependencies.
...
```python
import boto3
from flowforge_documents_s3 import S3DocumentPortInMemory, SQLiteDocumentIndex

port = S3DocumentPortInMemory(
	bucket="my-bucket",
	client=boto3.client("s3"),
	key_prefix="documents/",
	index_store=SQLiteDocumentIndex("/var/lib/flowforge/documents.sqlite3"),
)
...
- `S3DocumentPortInMemory(bucket, *, client, key_prefix, index_store, magic_validator)` — main adapter
- `SQLiteDocumentIndex(path)` — durable metadata and subject-index store for `index_store=...`
- `InMemoryDocumentIndex()` — default process-local metadata and subject-index store
...
| `index_store` | `InMemoryDocumentIndex()` | Metadata/subject index backend; use `SQLiteDocumentIndex(path)` for durable local metadata |
| `magic_validator` | built-in | Pass a no-op lambda to disable; pass a custom callable to extend |
| `allow_unvalidated_presigned_put` | `False` | Required opt-in for presigned PUT, which bypasses adapter-side magic-byte validation |
...
- **DS-02** (E-52): the canonical class is `S3DocumentPortInMemory`; without an explicit `index_store`, the subject index is process-local; the legacy name `S3DocumentPort` emits `DeprecationWarning` and maps to the same class
...
- **DS-05** (E-71): `SQLiteDocumentIndex` gives hosts a durable metadata and subject-index option, with tests proving metadata and subject attachments survive adapter restart while S3 remains the blob layer
...
- [`flowforge-signing-kms`](https://github.com/nyimbi/flowforge/tree/main/python/flowforge-signing-kms)
- [`flowforge-audit-pg`](https://github.com/nyimbi/flowforge/tree/main/python/flowforge-audit-pg)
