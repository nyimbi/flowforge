...
def test_nested_keys_sorted_recursively() -> None:
    out = canonical_json({"outer": {"z": 1, "a": {"y": 2, "b": 3}, "m": [1, 2]}})
    assert out == b'{"outer":{"a":{"b":3,"y":2},"m":[1,2],"z":1}}'


def test_pydantic_models_dump_to_json_mode() -> None:
    class _CanonicalFixture(BaseModel):
        id: str
        optional: str | None = None

    out = canonical_json(_CanonicalFixture(id="claim_intake"))
    assert out == b'{"id":"claim_intake","optional":null}'
