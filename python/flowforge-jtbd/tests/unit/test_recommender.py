...
def test_fit_rejects_when_frozen() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["claim intake"]).freeze()
    with pytest.raises(EmbeddingProviderFrozenError):
        p.fit(["payment recovery"])


def test_fit_skips_empty_token_documents() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["the and or", "claim intake"])
    assert p.vocab_size() == 2


async def test_freeze_captures_legacy_lazy_vocab() -> None:
    p = BagOfWordsEmbeddingProvider()
    await p.embed("claim intake")
    assert p.vocab_size() == 2
    p.freeze()
...
async def test_frozen_embed_rejects_unknown_tokens() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["claim intake"]).freeze()
    with pytest.raises(EmbeddingProviderFrozenError):
        await p.embed("payment recovery")


async def test_frozen_embed_uses_transform_for_known_tokens() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["claim intake"]).freeze()
    vec = await p.embed("claim")
    assert vec == p.transform("claim")


def test_unfreeze_allows_refit() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["claim intake"]).freeze()
    assert p.is_frozen()
    p.unfreeze()
...
async def test_transform_uses_legacy_vocab_before_freeze() -> None:
    p = BagOfWordsEmbeddingProvider()
    await p.embed("claim intake")
    vec = p.transform("claim")
    assert len(vec) == 2


def test_transform_without_vocab_returns_empty_vector() -> None:
    assert BagOfWordsEmbeddingProvider().transform("claim") == []


def test_transform_empty_text_returns_zero_vector_for_vocab() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["claim intake"])
    assert p.transform("") == [0.0, 0.0]


def test_transform_unknown_tokens_return_zero_vector_for_vocab() -> None:
    p = BagOfWordsEmbeddingProvider().fit(["claim intake"])
    assert p.transform("payment recovery") == [0.0, 0.0]
...
async def test_recommend_top_k_limits_results() -> None:
    ...
    for i in range(10):
        await rec.index_jtbd(
            f"j{i}", f"job {i} description text", metadata={"domain": "x"}
        )
...
async def test_recommend_domain_filter() -> None:
    rec = Recommender()
    await rec.index_jtbd(
        "ins_1", "insurance claim FNOL", metadata={"domain": "insurance"}
    )
    await rec.index_jtbd(
        "bank_1", "bank account deposit", metadata={"domain": "banking"}
    )
    results = await rec.recommend("claim account", top_k=5, domain_filter="insurance")
...
async def test_recommend_exclude_ids() -> None:
    rec = Recommender()
    await rec.index_jtbd(
        "source", "claim intake source text", metadata={"domain": "insurance"}
    )
    await rec.index_jtbd(
        "target", "claim intake similar text", metadata={"domain": "insurance"}
    )
    results = await rec.recommend("claim intake", top_k=5, exclude_ids={"source"})
...
async def test_recommend_for_jtbd_excludes_self() -> None:
    rec = Recommender()
    await rec.index_jtbd(
        "claim_intake", "file a claim FNOL insurance", metadata={"domain": "insurance"}
    )
    await rec.index_jtbd(
        "claim_review", "review a claim adjuster", metadata={"domain": "insurance"}
    )
...
async def test_recommend_rejects_zero_top_k() -> None:
    rec = Recommender()
    with pytest.raises(AssertionError, match="top_k"):
        await rec.recommend("anything", top_k=0)
...
async def test_build_recommender_accepts_jtbd_id_and_empty_text() -> None:
    rec = build_recommender(
        [
            {"jtbd_id": "fallback_id", "domain": None, "title": None},
        ]
    )
    results = await rec.recommend("fallback", top_k=1)
    assert results[0].jtbd_id == "fallback_id"
    ...
    specs = [
        {
            ...
            "domain": "insurance",
            "title": "File claim",
            "situation": "policyholder files FNOL",
            "motivation": "recover losses",
        ...
        },
        {
            ...
            "domain": "insurance",
            "title": "Review claim",
            "situation": "adjuster reviews FNOL",
            "motivation": "assess validity",
        ...
        },
        {
            ...
            "domain": "banking",
            "title": "Open account",
            "situation": "customer opens deposit",
