...
async def test_generate_ignores_non_text_blocks() -> None:
    client = _FakeClient()
    client.messages.create_response = _Response(
        content=[object(), {"text": 123}, _TextBlock(text="kept")],
    )
    provider = LlmProviderClaude(client=client)
    assert await provider.generate("x") == "kept"
...
async def test_stream_chat_handles_object_events_and_skips_non_text() -> None:
    client = _FakeClient()
    client.messages.stream_chunks = [
        _StreamEvent(delta=_Delta(text="object ")),
        SimpleNamespace(delta=SimpleNamespace(text=123)),
        object(),
        {"delta": "not-a-dict"},
        {"delta": {"text": None}},
        {"delta": {"text": "dict"}},
...
def test_constructor_builds_sdk_client_with_api_key(monkeypatch) -> None:
    ...
    provider = LlmProviderClaude(api_key="secret")

    assert provider._client is _FakeAsyncAnthropic.instances[0]  # noqa: SLF001
    assert _FakeAsyncAnthropic.instances[0].api_key == "secret"
