...
import flowforge_jtbd.templates.cache as cache_module
...
def _simple_template() -> JtbdTemplate:
    ...
    wf = {
        ...
        "transitions": [
            {"id": "close", "event": "close", "from_state": "open", "to_state": "done"}
        ],
    }
    t = JtbdTemplate(
        id="simple",
...
def test_template_optional_param_without_default_can_be_omitted() -> None:
    t = JtbdTemplate(
        id="optional",
        name="Optional",
        description="",
        parameters=[
            TemplateParameter(name="optional_note", type="string", required=False),
        ],
        workflow_template={"key": "static"},
    )
    assert t.instantiate({}) == {"key": "static"}


def test_template_extra_params_can_fill_forward_compatible_placeholders() -> None:
    t = JtbdTemplate(
        id="extra",
        name="Extra",
        description="",
        parameters=[],
        workflow_template={"extra": "{{future_param}}"},
    )
    assert t.instantiate({"future_param": "ok"}) == {"extra": "ok"}
...
def test_template_cache_list_templates_sorted() -> None:
    ...
    cache.register(t1)
    t2 = JtbdTemplate(
        id="aaa", name="A", description="", parameters=[], workflow_template={}
    )
    cache.register(t2)
...
def test_default_cache_returns_empty_when_library_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(cache_module, "__file__", str(tmp_path / "cache.py"))
    cache = TemplateCache.default()
    assert cache.size() == 0
