...
def test_actor_issue_for_unknown_spec_is_bucketed_as_bundle_issue() -> None:
    class _DetachedActorAnalyzer:
        def analyze(self, bundle: JtbdBundle) -> dict[str, list[Issue]]:
            return {
                "delegated_audit": [
                    Issue(
                        severity="warning",
                        rule="detached_actor_issue",
                        message="delegate not in bundle",
                    )
                ]
            }
    ...
    report = Linter(
        lifecycle=LifecycleAnalyzer(hint_optional=False),
        actors=_DetachedActorAnalyzer(),  # type: ignore[arg-type]
    ).lint(make_bundle([make_full_spec("x")]))
    assert [i.rule for i in report.bundle_issues] == ["detached_actor_issue"]
    assert all(
        i.rule != "detached_actor_issue"
