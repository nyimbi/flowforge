...
def test_observable_outcome_boosts_measurable_outcome() -> None:
    ...
    vague_outcome = score_jtbd(
        {**_good_spec(), "outcome": "Claim is handled properly."}
    )
    mo_good = next(d for d in good_outcome.dimensions if d.name == "measurable_outcome")
    mo_vague = next(
        d for d in vague_outcome.dimensions if d.name == "measurable_outcome"
    )
...
def test_long_narrative_field_gets_minor_clarity_penalty() -> None:
    long_situation = " ".join(f"detail{i}" for i in range(81))
    report = score_jtbd({**_good_spec(), "situation": long_situation})
    clarity = next(d for d in report.dimensions if d.name == "clarity")
    assert clarity.score < 100
    assert any("overly long" in finding for finding in clarity.findings)


def test_single_solution_coupled_phrase_scores_medium() -> None:
    report = score_jtbd(
        {
            **_good_spec(),
            "situation": "A policyholder needs help after a loss event.",
            "motivation": "Recover insured losses quickly.",
            "outcome": "A claim record is created.",
            "success_criteria": ["User clicks the submit button."],
        }
    )
    decoupling = next(d for d in report.dimensions if d.name == "solution_decoupling")
    assert decoupling.score == 70
...
def test_two_solution_coupled_phrases_score_low() -> None:
    report = score_jtbd(
        {
            **_good_spec(),
            "situation": "A policyholder needs help after a loss event.",
            "motivation": "Use the ClaimsHub system to recover losses.",
            "outcome": "A claim record is created.",
            "success_criteria": ["User clicks the submit button."],
        }
    )
    decoupling = next(d for d in report.dimensions if d.name == "solution_decoupling")
    assert decoupling.score == 40
...
async def test_scorer_score_async_without_llm_returns_heuristic() -> None:
    report = await QualityScorer().score_async(_good_spec())
    assert not report.llm_blended


async def test_scorer_score_async_falls_back_when_llm_raises() -> None:
    class RaisingLlm:
        async def generate(
            self,
...
async def test_scorer_score_async_handles_llm_without_json() -> None:
    class NonJsonLlm:
        async def generate(
            self,
            prompt: str,  # noqa: ARG002
    ...
    assert all("LLM assessment" in dim.findings[-1] for dim in report.dimensions)


def test_string_success_criteria_is_wrapped() -> None:
    report = score_jtbd(
        {
            **_good_spec(),
            "success_criteria": "Claim record created within 2 minutes.",
        }
    )
    actionability = next(d for d in report.dimensions if d.name == "actionability")
    assert actionability.score >= 65


def test_non_list_success_criteria_is_ignored() -> None:
    report = score_jtbd({**_good_spec(), "success_criteria": {"bad": "shape"}})
    actionability = next(d for d in report.dimensions if d.name == "actionability")
