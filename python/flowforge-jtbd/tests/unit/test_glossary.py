...
def test_conflicts_returns_empty_for_consistent_definitions() -> None:
    catalog = GlossaryCatalog()
    catalog.add_term(
        GlossaryTerm(term="case", domain="legal", definition="A litigation matter.")
    )
    catalog.add_term(
        GlossaryTerm(term="case", domain="gov", definition="A litigation matter.")
    )
...
def test_conflicts_flags_when_one_definition_missing() -> None:
    catalog = GlossaryCatalog()
    catalog.add_term(
        GlossaryTerm(term="intake", domain="insurance", definition="FNOL intake")
    )
...
def test_conflicts_no_issue_for_single_domain_term() -> None:
    catalog = GlossaryCatalog()
    catalog.add_term(
        GlossaryTerm(term="underwrite", domain="insurance", definition="Risk eval")
    )
    assert catalog.conflicts() == []
...
def test_from_bundle_extracts_data_capture_ids() -> None:
    ...
    catalog = GlossaryCatalog.from_bundle(make_bundle([spec]))
    entries = catalog.terms_for("policy_number")
    assert len(entries) == 1
    assert entries[0].domain == "insurance"
    assert entries[0].source_jtbd_ids == ["claim_intake"]


def test_from_bundle_ignores_non_list_data_capture() -> None:
    spec = JtbdLintSpec.model_validate(
        {
            "jtbd_id": "claim_intake",
            "version": "1.0.0",
            ...
            "data_capture": {"id": "policy_number"},
        }
    )
    catalog = GlossaryCatalog.from_bundle(make_bundle([spec]))
    assert catalog.terms_for("policy_number") == []
...
def test_rule_spec_level_warns_on_ambiguous_actor_role() -> None:
    catalog = GlossaryCatalog(
        [
            GlossaryTerm(
                term="adjuster", domain="insurance", definition="Claims handler"
            ),
