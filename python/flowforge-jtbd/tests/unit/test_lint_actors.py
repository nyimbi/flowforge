...
def test_role_undeclared_is_warning() -> None:
    ...
    assert all(
        i.severity == "warning" for i in out["x"] if i.rule == "actor_role_undeclared"
    )
...
def test_capacity_conflict_scoped_per_context() -> None:
    ...
    out = ActorConsistencyAnalyzer().analyze(bundle)
    for jtbd_id in ("a", "b"):
        conflicts = [i for i in out.get(jtbd_id, []) if i.rule == "actor_role_conflict"]
        assert conflicts == []
...
def test_different_non_conflicting_capacities_are_silent() -> None:
    a = make_full_spec(
        "a",
        actor=ActorRef(role="clerk", capacity="reviewer", context="claim"),
    )
    b = make_full_spec(
        "b",
        actor=ActorRef(role="clerk", capacity="observer", context="claim"),
    )
    bundle = make_bundle(
        [a, b],
        shared_roles={"clerk": RoleDef(name="clerk", default_tier=1)},
