...
def test_undo_is_optional_info_hint() -> None:
    ...
    assert any(
        i.rule == "optional_stage_recommended" and i.stage == "undo" for i in infos
    )


def test_delegated_optional_stage_suppresses_info_hint() -> None:
    delegate = JtbdLintSpec(
        jtbd_id="undo_log",
        version="1.0.0",
        stages=[
    ...
    spec = JtbdLintSpec(
        ...
        stages=[
            ...
            StageDecl(name="report"),
            StageDecl(name="audit"),
            StageDecl(name="undo", handled_by="undo_log"),
        ],
    )
    bundle = make_bundle([spec, delegate])
    issues = LifecycleAnalyzer().analyze(bundle, spec)
    assert all(
        not (i.rule == "optional_stage_recommended" and i.stage == "undo")
        for i in issues
    )
