...
class DependencyGraph:
    ...
    def _populate(self, bundle: JtbdBundle) -> None:
        ...
        assert len(known) == len(bundle.jtbds), (
            "duplicate jtbd_id in bundle — caller should dedupe before lint"
        )
    ...
    def _materialise_cycle(self, component: list[str]) -> DependencyCycle:
        ...
        while True:
            candidates = sorted(
                n for n in self.edges.get(current, []) if n in member_set
            )
            assert candidates, "every SCC node has at least one edge into the SCC"
    ...
    def _compute_topological_order(self) -> None:
        ...
        out_degree = {node: len(set(self.edges.get(node, []))) for node in self.edges}
        ready = sorted(node for node, degree in out_degree.items() if degree == 0)
        ...
        while ready:
            ...
            for dependent in reverse.get(node, []):
                remaining = {
                    target
                    for target in self.edges.get(dependent, [])
