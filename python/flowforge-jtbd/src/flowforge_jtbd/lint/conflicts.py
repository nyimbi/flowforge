...
class JtbdSemantics:
    ...
    def __post_init__(self) -> None:
        ...
        assert self.consistency in _CONSISTENCIES, (
...
_PAIR_RULES: tuple[_PairRule, ...] = (
    ...
    (
        ("realtime", "write", "strong"),
        ("batch", "write", "strong"),
        "error",
        "strong_consistency_in_batch_path",
    ),
    (
        ("realtime", "both", "strong"),
        ("batch", "write", "strong"),
        "error",
        "strong_consistency_in_batch_path",
    ),
...
def _emit_pair_issues(
    ...
    ent_sig: dict[tuple[str, _PairTuple], list[JtbdSemantics]] = defaultdict(list)
    for s in semantics:
        if not s.writes():
            continue
