...
def _score_clarity(situation: str, motivation: str, outcome: str) -> DimensionScore:
    ...
    for name, text in required.items():
        ...
        if wc < 5:
            ...
            findings.append(
                f"'{name}' may be overly long ({wc} words); consider tightening."
            )
            score -= 5
    ...
    if score >= 80:
        findings.insert(
            0, "All three narrative fields are present and reasonably sized."
        )
...
def _score_actionability(success_criteria: list[str]) -> DimensionScore:
    ...
    if not success_criteria:
        findings.append(
            "No success_criteria defined — add at least one measurable criterion."
        )
        return DimensionScore(name="actionability", score=0, findings=findings)

    score = 50  # base for having criteria at all
    findings.append(
        f"{len(success_criteria)} success criteri{'on' if len(success_criteria) == 1 else 'a'} found."
    )
    ...
    if measurable_count > 0:
        ...
        findings.append(
            f"{measurable_count} criteri{'on' if measurable_count == 1 else 'a'} contain measurable language."
        )
    else:
        findings.append(
            "Success criteria lack measurable language (numbers, time bounds, SLA thresholds)."
        )
        score -= 10
...
def _score_solution_decoupling(all_text: list[str]) -> DimensionScore:
    ...
    if coupled_count == 0:
        ...
        findings.append(
            f"{coupled_count} solution-coupled phrases — spec may describe UI/system rather than the job."
        )
        score = 40
    else:
        findings.append(
...
def _score_measurable_outcome(outcome: str) -> DimensionScore:
    ...
    if observable_count > 0:
        ...
        findings.append(
            "Outcome lacks observable language (e.g., 'record exists', 'was sent')."
        )
        score -= 20
    ...
    if vague_count > 0:
        ...
        findings.append(
            f"Outcome contains {vague_count} vague term(s) (e.g., 'good', 'properly')."
        )
