...
def _semver(value: str) -> str:
    ...
    if value.endswith("-") or value.endswith("+"):
        raise ValueError(f"version has empty pre-release/build suffix: {value!r}")
    try:
        Version(value)
...
def _id_pattern(value: str) -> str:
    ...
    """
    if not value or not value[0].isascii() or not ("a" <= value[0] <= "z"):
        raise ValueError("id must start with an ASCII lowercase letter")
    for ch in value:
        if not (
            ch.isascii() and (("a" <= ch <= "z") or ("0" <= ch <= "9") or ch == "_")
        ):
            raise ValueError(
...
def _hex_color(value: str) -> str:
    ...
    for ch in digits:
        if not (("0" <= ch <= "9") or ("a" <= ch.lower() <= "f")):
            raise ValueError(f"design colour {value!r} contains non-hex digit {ch!r}")
    return "#" + digits.lower()
