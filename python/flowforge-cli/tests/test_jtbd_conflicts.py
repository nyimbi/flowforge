"""Tests for ``flowforge_cli.jtbd.lint.conflicts`` (E-5).
...
# ---------------------------------------------------------------------------
# Pair rule: warning for combined consistency unclear
# ---------------------------------------------------------------------------
...
def test_pairs_combined_consistency_unclear_warns() -> None:
	...
	issues = solver.detect([_account_open(), _nightly_recompute()])
	assert len(issues) == 1
	issue = issues[0]
	assert issue.severity == "warning"
	assert issue.rule == "combined_consistency_unclear"
	assert issue.entity == "account"
	assert issue.jtbd_ids == ("account_open", "nightly_balance_recompute")


def test_pairs_strong_in_batch_path_errors() -> None:
	# Two strong-consistency writers on different timings → error.
	solver = PairsConflictSolver()
	issues = solver.detect([_account_open(), _nightly_recompute(consistency="strong")])
	assert len(issues) == 1
	assert issues[0].severity == "error"
	assert issues[0].rule == "strong_consistency_in_batch_path"
	assert issues[0].entity == "account"
...
def test_pairs_readers_not_flagged() -> None:
	# Two readers with mismatched (timing, consistency) on the same entity
	# don't conflict — read-only fan-out is fine.
	r1 = _read_balance()
	r2 = JtbdSemantics(
...
def test_pairs_one_reader_one_writer_skipped() -> None:
	# Reader-vs-writer doesn't produce a writer-vs-writer conflict.
...
def test_pairs_dedupes_across_shared_entities() -> None:
	# A pair touching two shared entities only yields one issue.
	a = JtbdSemantics(
		jtbd_id="a",
		timing="realtime",
		data="write",
	...
	# Entity is the alphabetically first shared one (stable).
	assert issues[0].entity == "e1"
...
def test_pairs_orders_errors_before_warnings() -> None:
	...
	# Expected output: 1 error + 1 warning, errors first.
	a = _account_open()
...
def test_default_solver_picks_z3_when_available() -> None:
	pytest.importorskip("z3", reason="python-z3-solver not installed")
	chosen = default_solver()
	assert chosen.backend == "z3"


def test_default_solver_falls_back_when_z3_missing(monkeypatch: pytest.MonkeyPatch) -> None:
	# Force the default-picker to think z3 is missing by stubbing the
	# import to raise. We do this in-module so the patch targets the
	# correct sys.modules namespace.
	import importlib
	import sys

	from flowforge_cli.jtbd.lint import conflicts as mod
	...
	# Wipe any cached `z3` so the import path actually re-runs.
...
def test_detect_conflicts_routes_through_default() -> None:
	# Smoke test: end-to-end public entrypoint emits the warning pair.
...
def test_detect_conflicts_partition_falls_back_to_pairs() -> None:
	...
	# All JTBDs are conflict-free pairs (same tuple), so we just check
	# the call returns without invoking Z3 on the large component (no
	# explicit assertion possible without monkeypatching, but the
	# coverage path is exercised).
	semantics = [
		JtbdSemantics(
			jtbd_id=f"j{i}",
			timing="realtime",
...
def test_conflict_issue_validates() -> None:
	# Smoke: well-formed issue constructs cleanly.
	issue = ConflictIssue(
		severity="error",
		rule="strong_consistency_in_batch_path",
		jtbd_ids=("a", "b"),
		entity="account",
	)
...
def test_conflict_issue_rejects_empty_jtbds() -> None:
	with pytest.raises(AssertionError):
		ConflictIssue(severity="error", rule="x", jtbd_ids=())
