...
#    wheel and sdist metadata keep internal Flowforge dependencies
#    bounded, runs `twine check`, smoke-installs every shipping wheel,
#    imports every shipping package, and runs the flowforge-cli console
#    entrypoint.
...
sdist `PKG-INFO` `Requires-Dist` metadata keep internal Flowforge dependencies
bounded, checks every wheel/sdist with `twine`, then installs every shipping
wheel from the built artifacts into a clean venv, imports every shipping
package, and runs `flowforge --help`.
