...
bounded and limited to shipping distributions, checks every wheel/sdist with
`twine`, then installs every shipping wheel from the built artifacts into a
clean venv, imports every shipping package, and runs `flowforge --help`.
