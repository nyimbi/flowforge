...
## PyPI artifact manifest metadata-identity audit

- Code review finding:
  - The standalone manifest verifier checked retained artifact filenames,
    counts, checksums, distribution identities, and release-version filenames,
    but did not inspect wheel `METADATA` or sdist `PKG-INFO`. A retained
    artifact could therefore carry mismatched internal `Name` or `Version`
    metadata while still matching its filename and checksum manifest.
- Action:
  - Added wheel `METADATA` and top-level sdist `PKG-INFO` parsing to
    `verify_pypi_artifact_manifest.py`.
  - The verifier now rejects artifacts whose embedded metadata `Name` does not
    match the artifact distribution or whose embedded metadata `Version` does
    not match the unified shipping release version.
  - Converted manifest-verifier tests to use minimal real wheel/sdist files and
    added a regression for metadata name/version drift.
- Verification:
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run ruff format scripts/audit_2026/verify_pypi_artifact_manifest.py tests/audit_2026/test_E_73_external_release_gate.py`:
    `1 file reformatted, 1 file left unchanged`.
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run ruff check scripts/audit_2026/verify_pypi_artifact_manifest.py tests/audit_2026/test_E_73_external_release_gate.py`:
  ...
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache make audit-2026-pypi-artifact-manifest`:
    verified the retained checksum manifest against repository `dist/` with
    package count, distribution identity, release-version, and embedded
...
## PyPI artifact manifest publication-metadata audit

- Code review finding:
  - The standalone manifest verifier inspected retained artifact names,
    versions, checksums, and required payload files, but it still did not compare
    publication-facing metadata against each package's `pyproject.toml`. A
    retained artifact could therefore preserve checksum integrity while
    presenting stale or incomplete Summary, Requires-Python, license, author,
    maintainer, keyword, classifier, project URL, or README metadata.
- Action:
  - Reused the PyPI build smoke's publication-metadata issue checker from
    `verify_pypi_artifact_manifest.py` so the standalone verifier enforces the
    same source-to-artifact metadata fidelity as the build gate.
  - Updated verifier fixture artifacts to include minimal valid publication
    metadata derived from the real `flowforge-core` `pyproject.toml`.
  - Added a retained-artifact regression for publication metadata drift.
- Verification:
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run ruff format scripts/audit_2026/verify_pypi_artifact_manifest.py tests/audit_2026/test_E_73_external_release_gate.py`:
    `1 file reformatted, 1 file left unchanged`.
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run ruff check scripts/audit_2026/verify_pypi_artifact_manifest.py tests/audit_2026/test_E_73_external_release_gate.py`:
  ...
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run pyright scripts/audit_2026/verify_pypi_artifact_manifest.py tests/audit_2026/test_E_73_external_release_gate.py`:
    `0 errors, 0 warnings, 0 informations`.
  - Focused publication metadata verifier tests:
    `2 passed`.
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run pytest tests/audit_2026/test_E_73_external_release_gate.py -q`:
    `54 passed`.
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache make audit-2026-pypi-artifact-manifest`:
    verified the retained checksum manifest against repository `dist/` with
    package count, distribution identity, release-version, embedded metadata
    identity, publication metadata fidelity, `py.typed`, wheel license, and
...
## External release evidence pass-list audit

- Code review finding:
  - The generated external release evidence summary listed PyPI artifacts and
    the checksum manifest under artifact contents, but its passing-evidence
    checklist did not explicitly state that the PyPI artifact build and
    retained-manifest verification passed. Release reviewers could therefore
    treat PyPI proof as an uploaded artifact rather than a completed gate.
- Action:
  - Added a generated evidence-summary pass line for PyPI artifact build and
    retained-manifest verification.
  - Added a workflow ratchet for that pass-list evidence line.
- Verification:
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run ruff format tests/audit_2026/test_E_73_external_release_gate.py`:
    `1 file left unchanged`.
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run ruff check tests/audit_2026/test_E_73_external_release_gate.py`:
  ...
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run pyright tests/audit_2026/test_E_73_external_release_gate.py`:
    `0 errors, 0 warnings, 0 informations`.
  - Focused workflow evidence pass-list ratchet:
    `1 passed`.
  - `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache uv run pytest tests/audit_2026/test_E_73_external_release_gate.py -q`:
    `56 passed`.
- Remaining risk:
