...
- External release CI wiring:
  ...
  - Result: PR run `26082912970` registered `audit-2026-release-external` but failed at `Require UMS backend checkout token` with an explicit `UMS_BACKEND_TOKEN` prerequisite error, proving the remaining CI blocker is missing backend access, not workflow registration.
