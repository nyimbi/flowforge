...
| Keep GitHub CI runnable from a source checkout | Workflow setup now uses tracked `pyproject.toml` cache dependency inputs instead of ignored `uv.lock`, and pnpm 11 jobs run on Node 22; the external-release ratchet covers both requirements | Done |
