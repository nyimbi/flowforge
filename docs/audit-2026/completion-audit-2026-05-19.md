...
| Run local full gate | `UV_CACHE_DIR=/private/tmp/flowforge-uv-cache VISREG_ALLOW_SKIP=1 bash scripts/check_all.sh` -> `U24 gate PASSED`, 46 Python packages, 7 JS packages, 2,664 counted tests/assertions, elapsed 97s on the latest rerun | Done with documented local visual skip |
...
| Run audit ratchets | `uv run pytest tests/audit_2026 -q --tb=short` -> `198 passed`, including the new workflow helper ratchets and YAML parse coverage | Done |
...
- `GOPATH=/private/tmp/flowforge-go GOMODCACHE=/private/tmp/flowforge-go/pkg/mod GOCACHE=/private/tmp/flowforge-go/build-cache go run github.com/rhysd/actionlint/cmd/actionlint@latest .github/workflows/audit-2026-dom-baselines.yml .github/workflows/audit-2026-polish-copy-sidecar.yml .github/workflows/audit-2026-release-external.yml`
  passed, so the new helper/release workflow YAML is validated by both
  `actionlint` and the audit ratchets that inspect required workflow structure,
  secrets, commands, artifacts, and skip-flag absence.
