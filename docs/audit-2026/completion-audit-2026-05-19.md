...
| Keep GitHub CI runnable from a source checkout | Workflow setup now uses tracked `pyproject.toml` cache dependency inputs instead of ignored `uv.lock`, pnpm 11 jobs run on Node 22, and JTBD lint passes repo-relative bundle paths to the CLI; the external-release ratchet covers these requirements | Done |
