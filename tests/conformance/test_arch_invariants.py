...
def test_invariant_5_cross_runtime_parity() -> None:
	...
	Acceptance criterion (audit-fix-plan §4.2 JS-01, JS-02):
	  * 250-input cross-runtime fixture file → identical guard outputs.

	The canonical fixture is ``expr_parity_v2.json`` (250 cases: 200
	base + 50 ``conditional``-tagged cases that cover the W1
	``form_renderer = "real"`` ``show_if`` shapes). The legacy
	``expr_parity_200.json`` was retired in v0.3.0 W3 once v2 had
	stayed green across W1 + W2 per the engineering plan §11.1.
	...
	fixture is regenerated by `generate_fixture.py` from the live Python
	...
	cases = data["cases"]
	assert len(cases) == 250, f"fixture must hold exactly 250 cases, got {len(cases)}"
