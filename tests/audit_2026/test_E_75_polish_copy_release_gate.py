...
_BUNDLE = _GATE._load_bundle(
    _ROOT / "examples" / "insurance_claim" / "jtbd-bundle.json"
)
_VALID_SIDE_CAR = {
...
def test_polish_copy_sidecar_authoring_workflow_uploads_review_candidate() -> None:
    ...
    assert (
        "hashFiles('examples/insurance_claim/jtbd-bundle.json.overrides.json') != ''"
        in workflow
    )
    assert (
...
def test_polish_copy_sidecar_gate_rejects_stale_prompt_hash() -> None:
    payload = dict(_VALID_SIDE_CAR, prompt_sha256="b" * 64)
    sidecar = JtbdCopyOverrides.model_validate(payload)
