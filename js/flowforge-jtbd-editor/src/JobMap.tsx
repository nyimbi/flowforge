...
const toReactFlowNode = (n: NodeLayout): Node => ({
	id: n.jtbdId,
	...
	style: {
		...
		height: n.height,
		borderRadius: 8,
		border: `2px solid ${n.inCycle ? CYCLE_NODE_BORDER : NODE_BORDER}`,
		background: "#ffffff",
		fontSize: 12,
		display: "flex",
		alignItems: "center",
...
export const JobMap = ({
	...
	const nodes = useMemo(() => layout.nodes.map(toReactFlowNode), [layout.nodes]);
...
const JobMapNode = ({ node, onSelect }: JobMapNodeProps): JSX.Element => {
	const borderColor = node.inCycle ? CYCLE_NODE_BORDER : NODE_BORDER;
